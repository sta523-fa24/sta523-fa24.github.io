...
```shell
cr173@hypatia [class_2024_03_22]$ sqlite3 employees.sqlite
...
but becomes more useful when used with aggregation functions like `COUNT()`
...
Note that SQLite does not directly support an `OUTER JOIN` (e.g a full join in dplyr) or a `RIGHT JOIN`.
