...
The format of duckdb's output (in the CLI) is controled via `.mode` -  the default is `duckbox`, see a list of [other modes](https://duckdb.org/docs/api/cli/output_formats.html). 
...
#| eval: false
#| code-line-numbers: false
D SELECT name AS first_name, salary FROM employees ORDER BY salary;
## ┌────────────┬─────────┐
## │ first_name │ salary  │
## │  varchar   │ double  │
## ├────────────┼─────────┤
...
and order results with `ORDER BY` with or without `DESC`

::: {.medium}
```{sql}
#| eval: false
#| code-line-numbers: false
D SELECT * FROM employees ORDER BY name DESC LIMIT 3;
## ┌─────────┬──────────────────┬─────────┬────────────┐
## │  name   │      email       │ salary  │    dept    │
## │ varchar │     varchar      │ double  │  varchar   │
## ├─────────┼──────────────────┼─────────┼────────────┤
...
Using duckdb calculate the following quantities,
...
DuckDB has a neat trick in that it can treat files as tables (for supported formats), this lets you query them without havign to explicitly read them into the database and create a table.
