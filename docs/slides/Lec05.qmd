...
Matrices in R use column major ordering (data is stored in by column).
...
When creating a data frame from a set of vectors, the lengths of the component vectors will be coerced to match. However, if they not multiples of each other then there will be an error (other previous forms of length coercion would produce a warning for this case).
...
::: {.medium}
Below are 100 values, write down how you would create a subset to accomplish each of the following:
